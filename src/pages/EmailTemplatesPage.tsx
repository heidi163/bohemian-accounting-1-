...
نرفق لكم الفاتورة رقم {{invoice_number}} بتاريخ {{invoice_date}} بإجمالي {{total_amount}} {{currency}}.

تاريخ الاستحقاق: {{due_date}}

شكراً لتعاملكم معنا.
مع تحيات فريق {{company_name}}`,
  ...
  },
  {
    ...
    body: `تذكير هام بالسداد،

عزيزي العميل،
نودّ تذكيركم بأن الفاتورة رقم {{invoice_number}} بمبلغ {{amount_due}} {{currency}} تستحق السداد بتاريخ {{due_date}}.

يرجى التكرم بالسداد في أقرب وقت لتجنب غرامات التأخير.

مع تحيات قسم الحسابات.`,
  ...
  },
  {
    ...
    body: `عزيزي {{employee_name}}،

نرفق لكم تفاصيل الراتب الخاص بشهر {{period}}:

- الراتب الأساسي: {{basic_salary}} ج.م
- البدلات والمكافآت: {{allowances}} ج.م
- الاستقطاعات: -{{deductions}} ج.م
----------------------------
الصافي المستحق: {{net_salary}} ج.م
...
قسم الموارد البشرية`,
  ...
  },
  {
    ...
    body: `السادة أعضاء الإدارة الموقرين،

إليكم الملخص المالي ليوم {{date}}:

- إجمالي الفواتير الصادرة: {{invoices_issued}}
- المدفوعات المستلمة: {{payments_received}} ج.م
- المصروفات المسجلة: {{expenses_recorded}} ج.م
- الرصيد النقدي الحالي: {{cash_balance}} ج.م
